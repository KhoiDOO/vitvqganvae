...
import os
import tempfile
from vitvqganvae.model.cnn.variant import FlexVQVAE, FlexVQVAEConfig

class TestFlexVQVAE(unittest.TestCase):
    def setUp(self):
        self.dim = 64
        self.in_channel = 3
        self.out_channel = 3
        ...
        self.num_in_res_blocks = 1
        self.num_out_res_blocks = 1
        self.group = 16
        self.codebook_size = 16
        self.image_size = 32
        self.batch_size = 2
        self.device = torch.device('cpu')
        self.vqvae = FlexVQVAE(
            dim=self.dim,
            in_channel=self.in_channel,
            out_channel=self.out_channel,
            layers=self.layers,
            layer_mults=self.layer_mults,
            num_in_res_blocks=self.num_in_res_blocks,
            num_out_res_blocks=self.num_out_res_blocks,
            group=self.group,
    ...
    def test_properties(self):
        ...
        self.assertEqual(self.vqvae.num_in_res_blocks, self.num_in_res_blocks)
        self.assertEqual(self.vqvae.num_out_res_blocks, self.num_out_res_blocks)
        ...
        self.assertIsInstance(self.vqvae.copy_for_eval(), FlexVQVAE)
