import unittest
import torch
from vitvqganvae.model.vit.decoder import ImgVITDecoder

class TestImgVITDecoder(unittest.TestCase):
    def setUp(self):
    ...
    def test_forward_shape(self):
        ...
        self.assertEqual(out.shape, (batch_size, self.out_channel, *self.image_size))
    ...
    def test_get_last_layer(self):
        ...
        # ConvTranspose2d weight shape: (out_channel, dim, kernel_h, kernel_w)
        self.assertEqual(last_layer.shape[0], self.dim)
        self.assertEqual(last_layer.shape[1], self.out_channel)

    def test_properties(self):
        self.assertEqual(self.decoder.image_size, self.image_size)
        self.assertEqual(self.decoder.patch_size, self.patch_size)
        self.assertEqual(self.decoder.out_channel, self.out_channel)
        ...
        num_patches = (self.image_size[0] // self.patch_size[0]) * (self.image_size[1] // self.patch_size[1])
        patch_dim = self.out_channel * self.patch_size[0] * self.patch_size[1]
    ...
    def test_invalid_patch_size(self):
        with self.assertRaises(AssertionError):
            ImgVITDecoder(image_size=(32, 32), patch_size=(7, 7))

    def test_causal_kwarg(self):
        with self.assertRaises(AssertionError):
            ImgVITDecoder(image_size=(32, 32), patch_size=(8, 8), causal=True)
